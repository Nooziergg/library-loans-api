...
| Audit trail | `AuditSaveChangesInterceptor` — every insert, update and delete recorded with actor, correlation id and before/after values, in the same transaction as the change. Hung off `SaveChanges` rather than called from handlers, so coverage is structural: a change cannot reach the database without passing through it, and a new aggregate is audited the day it is mapped | **BUILT** |
